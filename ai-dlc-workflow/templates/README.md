...
- **`enhancement-request.md`** — copy to `/ai-dlc/requests/<change-name>.md` for anything genuinely new or
  ...
  tasks) — directly into `/ai-dlc/stories.md`, tagged `Status: Not Started | In Progress |
...
- **`bug-request.md`** — copy to `/ai-dlc/requests/<bug-name>.md` for the lightweight bug-fix variant. Claude
  analyzes it in Plan Mode (root cause, fix approach, tasks) directly as the approval step — no separate
  file for that either. `/ai-dlc/learnings.md` is the durable record afterward (mandatory for bug fixes),
  plus `stories.md`/`business-rules.md` if the fix corrects something already documented there.

Both assume `/ai-dlc/` already exists and has been accepted — see `ai-dlc-workflow/brownfield/`
